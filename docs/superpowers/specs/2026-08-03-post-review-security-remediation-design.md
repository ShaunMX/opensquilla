...
Each unauthenticated guest turn receives a fresh managed temporary workspace beneath a deterministic OpenSquilla guest root derived as a sibling of `config.state_dir`. It never adopts the configured agent workspace or a browser-supplied path, and it is deliberately outside the private state authority root. The workspace, guest HOME, and guest temp directories are read/write. The anonymous model-visible tool surface is a hard default-deny allowlist containing only reviewed managed-workspace file tools and guarded network tools. Process tools and bundled runtimes are not exposed to anonymous guests on any platform; bundled runtimes remain available to local owners and valid named Tokens in Safe mode. Windows additionally retains the trusted process gate as defense in depth before support checks, ACL projection, or launch. A valid named Token may widen this according to its capabilities.
